...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    return max(0,1-(label*(np.matmul(feature_vector,theta)+theta_0)))
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        sum_hinge_loss+=max(0,1-(labels[i]*(np.matmul(feature_matrix[i],theta)+theta_0)))

    avg_hinge_loss=sum_hinge_loss/len(feature_matrix)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    return (sum_theta/count_changes,current_theta_0)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    # for t in range(T):
    #     for i in range(feature_matrix.shape[0]): #mit.get_order(feature_matrix.shape[0]):
    #         count_changes=count_changes+1
    #         eta=1/np.sqrt(count_changes)
    #         (current_theta,current_theta_0)=pegasos_single_step_update(feature_matrix[i],labels[i],L,eta,current_theta,current_theta_0)
    #         if abs(current_theta_0)<epsilon:
    #             current_theta_0=0
    #         current_theta[abs(current_theta) < epsilon] = 0
...
def bag_of_words(texts):
    ...
    """
    # Your code here
    sw_df=pd.read_csv('stopwords.txt',header=None)
    sw_list=sw_df[0].values.tolist()
